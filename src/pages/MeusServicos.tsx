...
import { Search, Wrench, Clock, CheckCircle2 } from 'lucide-react';
...
const MeusServicos = () => {
  ...
  // Available orders (unassigned, waiting for quote)
  const availableOrders = mockServiceOrders.filter(
    os => os.status === 'aguardando_orcamento' && !os.tecnicoId
  );

  const pendingOrders = [...technicianOrders, ...availableOrders].filter(
    os => os.status === 'aguardando_orcamento'
  );

  const inProgressOrders = technicianOrders.filter(
    os => os.status === 'aprovado_aguardando' || os.status === 'em_execucao'
  );

  const completedOrders = technicianOrders.filter(
    os => os.status === 'concluido'
  );

  const filterOrders = (orders: typeof mockServiceOrders) => {
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Gerencie seus orçamentos e execuções
          </p>
        </div>
      </div>
          ...
          {filterOrders(pendingOrders).length > 0 ? (
            ...
            <div className="os-card text-center py-12">
              ...
              <p className="text-muted-foreground">
                Nenhum serviço aguardando orçamento
              </p>
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="in-progress" className="space-y-4 stagger-children">
          {filterOrders(inProgressOrders).length > 0 ? (
            ...
            <div className="os-card text-center py-12">
              ...
              <p className="text-muted-foreground">
                Nenhum serviço para executar no momento
              </p>
            </div>
          )}
        </TabsContent>
